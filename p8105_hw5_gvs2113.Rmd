...
sim_results_df = 
  expand_grid(
    n = 30, 
    mu = 0:6,
    iter = 1:50 #change to 5000 later
  ) |> 
...
When comparing these data, the plot generated for all of the data is much closer to being linear than the plot generated from the estimates in which the null was rejected. As the true value of mu increases, the two lines come together and converge when the mu value is 5 and 6. The largest difference between the two values is when mu is 1. 
